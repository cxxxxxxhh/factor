...
class GTJA_191:
    ...
    def __init__(self, end_date, index):
        security = self.get_index_stocks(index)
        price = self.get_price(security, None, end_date, ['open','close','low','high','avg_price','prev_close','volume','turnover'],is_panel=1)
        benchmark_price = self.get_price(index, None, end_date, '1d',['open','close','low','high','avg_price','prev_close','volume'],is_panel=1)
    ...
    def uploaddata(self):
        self.amount_adf0 = pd.read_csv('amount_adf0.csv')
        self.amount_adf0.set_index(pd.to_datetime(self.amount_adf0['date'],format='%Y-%m-%d'),inplace=True)
        self.symbol_list=pd.read_csv('symbol_list.csv')
        self.symbol_list.set_index(pd.to_datetime(self.symbol_list['date'],format='%Y-%m-%d'),inplace=True)
        self.mktcap_adf0=pd.read_csv('mktcap_adf0.csv')
        self.mktcap_adf0.set_index(pd.to_datetime(self.mktcap_adf0['date'],format='%Y-%m-%d'),inplace=True)
        self.turnover_adf0=pd.read_csv('turnover_adf0.csv')
        self.turnover_adf0.set_index(pd.to_datetime(self.turnover_adf0['date'],format='%Y-%m-%d'),inplace=True)
        self.close_adf2=pd.read_csv('close_adf2.csv')
        self.close_adf2.set_index(pd.to_datetime(self.close_adf2['date'],format='%Y-%m-%d'),inplace=True)
        self.high_adf2=pd.read_csv('high_adf2.csv')
        self.high_adf2.set_index(pd.to_datetime(self.high_adf2['date'],format='%Y-%m-%d'),inplace=True)
        self.low_adf2=pd.read_csv('low_adf2.csv')
        self.low_adf2.set_index(pd.to_datetime(self.low_adf2['date'],format='%Y-%m-%d'),inplace=True)
        self.open_adf2=pd.read_csv('open_adf2.csv')
        self.open_adf2.set_index(pd.to_datetime(self.open_adf2['date'],format='%Y-%m-%d'),inplace=True)
        self.shareoutstanding_adf2=pd.read_csv('sharesoutstanding_adf2.csv')
        self.shareoutstanding_adf2.set_index(pd.to_datetime(self.shareoutstanding_adf2['date'],format='%Y-%m-%d'),inplace=True)
        self.volume_adf2=pd.read_csv('volume_adf2.csv')
        self.volume_adf2.set_index(pd.to_datetime(self.volume_adf2['date'],format='%Y-%m-%d'),inplace=True)
        self.vwap_adf2=pd.read_csv('vwap_adf2.csv')
        self.vwap_adf2.set_index(pd.to_datetime(self.vwap_adf2['date'],format='%Y-%m-%d'),inplace=True)
        self.ipo3mon=pd.read_csv('ipo3mon.csv')
        self.ipo3mon.set_index(pd.to_datetime(self.ipo3mon['date'],format='%Y-%m-%d'),inplace=True)
        self.stInfo=pd.read_csv('stInfo.csv')
        self.stInfo.set_index(pd.to_datetime(self.stInfo['date'],format='%Y-%m-%d'),inplace=True)
        self.prev_close_adf2=pd.read_csv('pre_close_adf2.csv')
        self.prev_close_adf2.set_index(pd.to_datetime(self.prev_close_adf2['date'],format='%Y-%m-%d'),inplace=True)

    def get_index_stocks(self,index):
        self.uploaddata()
        #symbol_list = pd.read_csv('symbol_list.csv')
        return self.symbol_list.iloc[:,0]
    
    def get_price(self,security,start_date,end_date,fields, is_panel):
        # 这里假设 price_data 是一个包含价格数据的字典，其中键是证券代码，值是一个包含各个字段价格数据的 DataFrame 或者 Panel 对象
        price_data = {}
        for stock in security:
            price_data[stock] = {}
            for field in fields:
                # 假设你的本地数据是一个DataFrame，包含每个股票的价格数据
                # 这里使用loc函数获取指定时间段内的价格数据
                # price_data[stock][field] = local_data.loc[start_date:end_date, stock][field].tolist()
                #['open','close','low','high','avg_price','prev_close','volume','turnover']
                if field == 'open':
                    price_data[stock][field] = self.open_adf2.loc[start_date:end_date, stock].tolist()
                if field == 'close':
                    price_data[stock][field] = self.close_adf2.loc[start_date:end_date, stock].tolist()
                if field == 'low':
                    price_data[stock][field] = self.low_adf2.loc[start_date:end_date, stock].tolist()
                if field == 'high':
                    price_data[stock][field] = self.high_adf2.loc[start_date:end_date, stock].tolist()
                if field == 'avg_price':
                    price_data[stock][field] = self.vwap_adf2.loc[start_date:end_date, stock].tolist()
                if field == 'prev_close':
                    price_data[stock][field] = self.prev_close_adf2.loc[start_date:end_date, stock].tolist()
                if field == 'volume':
                    price_data[stock][field] = self.volume_adf2.loc[start_date:end_date, stock].tolist()
                if field == 'turnover':
                    price_data[stock][field] = self.turnover_adf0.loc[start_date:end_date, stock].tolist()
        # 将面板数据存储为DataFrame
        # price_data = pd.Panel(price_data)

        if is_panel:  # 如果 is_panel 参数为 True，则返回 Panel 对象
            panel_data = {}
            for field in fields:
                panel_data[field] = {}
                for security_code, data in price_data.items():
                    panel_data[field][security_code] = data[field]

            return panel_data

        else:  # 如果 is_panel 参数为 False，则返回 DataFrame 对象
            # 在这里根据需要处理数据，例如根据证券代码筛选数据，按照日期范围进行过滤等
            # 这里简单起见，直接返回一个包含所有证券数据的 DataFrame 对象
            # price_data = pd.Panel(price_data)
