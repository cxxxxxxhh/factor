...
class GTJA_191:
    ...
    def __init__(self):
        print("正在从文件中获取源数据")
        self.open_all = pd.read_csv("data/open.csv").set_index('date', inplace=False)
        self.close_all = pd.read_csv("data/close.csv").set_index('date', inplace=False)
        self.low_all = pd.read_csv("data/low.csv").set_index('date', inplace=False)
        self.high_all = pd.read_csv("data/high.csv").set_index('date', inplace=False)
        self.avg_all = pd.read_csv("data/average.csv").set_index('date', inplace=False)
        self.prev_all = pd.read_csv("data/previous.csv").set_index('date', inplace=False)
        self.volume_all = pd.read_csv("data/volume.csv").set_index('date', inplace=False)
        self.amount_all = pd.read_csv("data/amount.csv").set_index('date', inplace=False)
        #self.data_all=pd.read_csv("data/date.csv").set_index('date', inplace=False)
        self.benchmark_all = pd.read_csv("data/benchmark.csv").set_index('date', inplace=False)
        print("Done！")
        ...
        self.set_date_and_stock()

    def set_date_and_stock(self, start_date='2013-01-04', end_date='2024-03-04',
                           start_stock='000001.SZ', end_stock='689009.SH',
                           date_list=[], stock_list=[]):
        print("正在截取所需数据，默认为全部")
        if len(date_list) == 0:
            open_chosen = self.open_all.loc[start_date:end_date, :]
            close_chosen = self.close_all.loc[start_date:end_date, :]
            low_chosen = self.low_all.loc[start_date:end_date, :]
            high_chosen = self.high_all.loc[start_date:end_date, :]
            avg_chosen = self.avg_all.loc[start_date:end_date, :]
            prev_chosen = self.prev_all.loc[start_date:end_date, :]
            volume_chosen = self.volume_all.loc[start_date:end_date, :]
            amount_chosen = self.amount_all.loc[start_date:end_date, :]
            #date_chosen=self.data_all.loc[start_date:end_date, :]
            self.benchmark_open_price = self.benchmark_all.loc[start_date:end_date, 'open']
            self.benchmark_close_price = self.benchmark_all.loc[start_date:end_date, 'close']
        else:
            open_chosen = self.open_all.loc[date_list, :]
            close_chosen = self.close_all.loc[date_list, :]
            low_chosen = self.low_all.loc[date_list, :]
            high_chosen = self.high_all.loc[date_list, :]
            avg_chosen = self.avg_all.loc[date_list, :]
            prev_chosen = self.prev_all.loc[date_list, :]
            volume_chosen = self.volume_all.loc[date_list, :]
            amount_chosen = self.amount_all.loc[date_list, :]
            #date_chosen=self.date_all.loc[date_list, :]
            self.benchmark_open_price = self.benchmark_all.loc[date_list, 'open']
            self.benchmark_close_price = self.benchmark_all.loc[date_list, 'close']
        if len(stock_list) == 0:
            self.open_price = open_chosen.loc[:, start_stock:end_stock]
            self.close = close_chosen.loc[:, start_stock:end_stock]
            self.low = low_chosen.loc[:, start_stock:end_stock]
            self.high = high_chosen.loc[:, start_stock:end_stock]
            self.avg_price = avg_chosen.loc[:, start_stock:end_stock]
            self.prev_close = prev_chosen.loc[:, start_stock:end_stock]
            self.volume = volume_chosen.loc[:, start_stock:end_stock]
            self.amount = amount_chosen.loc[:, start_stock:end_stock]
            #self.date=date_chosen
        else:
            self.open_price = open_chosen.loc[:, stock_list]
            self.close = close_chosen.loc[:, stock_list]
            self.low = low_chosen.loc[:, stock_list]
            self.high = high_chosen.loc[:, stock_list]
            self.avg_price = avg_chosen.loc[:, stock_list]
            self.prev_close = prev_chosen.loc[:, stock_list]
            self.volume = volume_chosen.loc[:, stock_list]
            self.amount = amount_chosen.loc[:, stock_list]
            #self.date=date_chosen
        print("Done！\n当前数据中日期总数为：%d，股票总数为：%d" % (self.open_price.shape[0], self.open_price.shape[1]))
        return "Done!"
    #############################################################################

    def alpha_001(self):
        temp=np.log(self.volume)
        data1 = temp.diff(periods=1).rank(axis=1, pct=True)
    ...
    def alpha_005(self):
        ...
        ts_volume = (self.volume.iloc[-5:, :]).rank(axis=0, pct=True)
        ts_high = (self.high.iloc[-5:, :]).rank(axis=0, pct=True)
        corr_ts = ts_high.rolling(window=3).corr(ts_volume)
    ...
    def alpha_006(self):
        ...
        condition1 = ((self.open_price * 0.85 + self.high * 0.15).diff(4) > 0)
        condition2 = ((self.open_price * 0.85 + self.high * 0.15).diff(4) == 0)
        condition3 = ((self.open_price * 0.85 + self.high * 0.15).diff(4) < 0)
    ...
    def alpha_016(self):
        ...
        alpha = -result.rank(axis=1,pct=True).max()
        return alpha.dropna()
...
#3*SMA((CLOSE-TSMIN(LOW,9))/(TSMAX(HIGH,9)-TSMIN(LOW,9))*100,3,1)-2*SMA(SMA((CLOSE-TSMIN(LOW,9))/(MAX(HIGH,9)-TSMAX(LOW,9))*100,3,1),3,1)
        ...
        part1 = 3 * (100 *temp1/ temp2).ewm(1.0 / 3).mean()
        temp3 =   (100*temp1/ temp2).ewm(1.0 / 3).mean()
        part2 = 2 * temp3.ewm(1.0 / 3).mean()
        result = part1 - part2
        alpha = result.iloc[-1, :]
        return alpha.dropna()
...
#((RANK(DECAYLINEAR(DELTA((CLOSE), 2),8)) - RANK(DECAYLINEAR(CORR(((VWAP * 0.3) + (OPEN * 0.7)),SUM(MEAN(VOLUME,180), 37), 14), 12))) * -1)
    ...
    def alpha_044(self):
        ...
        temp2 = temp2.iloc[-m:, :]
        part2 = temp2.apply(lambda x: x * weight2)
        part2 = part1.iloc[-15:, ].rank(axis=0, pct=True)
...
#SIGN((DELAY(CLOSE, 2) - DELAY(CLOSE, 3)))))) * SUM(VOLUME, 5)) / SUM(VOLUME, 20))
    ...
    def alpha_057(self):
        ...
        result = (100 * part1 / part2).ewm(1.0 / 3).mean()
        alpha = result.iloc[-1, :]
        return alpha.dropna()
    ...
    def alpha_063(self):
        ...
        part1 = part1.ewm(1.0 / 6).mean()
        part2 = (self.close - self.close.shift()).abs()
        part2 = part2.ewm(1.0 / 6).mean()
    ...
    def alpha_064(self):
        ...
        n = 4
        m = 14
        temp1 = (self.avg_price.rank(axis=1, pct=True)).rolling(4).corr(self.volume.rank(axis=1, pct=True))
        temp1 = temp1.iloc[-n:, :]
        seq1 = [2 * i / (n * (n + 1)) for i in range(1, n + 1)]
        seq2 = [2 * i / (m * (m + 1)) for i in range(1, m + 1)]
        weight1 = np.array(seq1)
        ...
        temp2 = self.close.rank(axis=1, pct=True)
    ...
    def alpha_066(self):
        ...
        part1 = self.close-self.close.rolling(6).mean()
        part2=self.close.rolling(6).mean()
    ...
    def alpha_067(self):
        ...
        part1 = part1.ewm(1.0 / 24).mean()
        temp2 = temp1.abs()
        part2 = temp2.ewm(1.0 / 24).mean()
        result = part1 * 100 / part2
        alpha = result.iloc[-1, :].dropna()
        return alpha
    ...
    def alpha_071(self):
        ...
        #
        data =100* (self.close - self.close.rolling(24).mean()) / self.close.rolling(24).mean()
    ...
    def alpha_072(self):
        ...
        alpha = ((data1 / data2 * 100).ewm(1 / 15).mean()).iloc[-1].dropna()
        return alpha
    ...
    def alpha_074(self):
        ...
        rank1 = data1.rolling(7).corr(data2).rank(axis=1, pct=True)
        data3 = self.avg_price.rank(axis=1, pct=True)
        data4 = self.volume.rank(axis=1, pct=True)
        rank2 = data3.rolling(6).corr(data4).rank(axis=1, pct=True)
    ...
    def alpha_075(self):
        ...
        # COUNT(CLOSE>OPEN & BANCHMARKINDEXCLOSE<BANCHMARKINDEXOPEN,50)/COUNT(BANCHMARKINDEXCLOSE<BANCHMARKINDEXOPEN,50)
        # benchmark = self.get_price('000001.SH', None, end_date,  ['open','close'], False)
        # Todo : self.amount_adf0????
        # index=self.amount_adf0.index.get_loc(end_date)
        # benchmark = self.get_price('000001.SH', index - 50 , end_date,['open','close'],is_panel=0)
        s1 = self.open_price.loc[:end_date, '000001.SZ']
        s2 = self.close.loc[:end_date, '000001.SZ']
        benchmark = pd.DataFrame(list(zip(s1, s2)))
        benchmark.columns = ['open', 'close']
    ...
    def alpha_076(self):
        ...
        data1 = ((self.close /self.close.shift() - 1).abs() )/ self.volume
        data2=data1.iloc[-20:].std()
        data3=data1.iloc[-20:].mean()
        alpha = (data2 / data3).dropna()
        return alpha
    ...
    def alpha_079(self):
        ...
        #
        data1 = np.maximum((self.close - self.close.shift()), 0).ewm(1 / 12).mean()
        data2 = (self.close - self.close.shift()).abs().ewm(1 / 12).mean()
    ...
    #############################################################################
    def alpha_081(self):
        result = self.volume.ewm(2.0 / 21).mean()
        alpha = result.iloc[-1, :].dropna()
        return alpha
    ...
    def alpha_082(self):
        ...
        part1 = self.high.rolling(6).max() - self.close
        part2 = self.high.rolling(6).max() - self.low.rolling(6).min()
        result = (100 * part1 / part2).ewm(1.0 / 2.0).mean()
        alpha = result.iloc[-1, :].dropna()
        return alpha
...
#(OPEN - ((HIGH + LOW) / 2))), 11), 7)) * -1)
    ...
    def alpha_092(self):
        ...
        delta = (self.close * 0.35 + self.avg_price * 0.65).shift(2)
        rank1 = (delta.rolling(window=3).apply(func_decaylinear)).rank(axis=1, pct=True)
        temp = ((self.volume.rolling(window=180).mean()).rolling(13).corr(self.close)).abs()
        rank2 = (temp.rolling(window=5).apply(func_decaylinear)).iloc[-15:].rank()
        # cond_max = rank1 > rank2
        # rank2[cond_max] = rank1[cond_max]
        # alpha = (-rank2).iloc[-1, :]
        alpha=np.maximum(rank1,rank2)
    ...
    def alpha_096(self):
        ...
        temp = 100 * (self.close - self.low.rolling(9).min()) / (self.high.rolling(9).max() - self.low.rolling(9).min())
        sma1 = temp.ewm(span=3, adjust=False).mean()
        alpha = (sma1.ewm(span=3, adjust=False).mean()).iloc[-1, :]
        alpha = alpha.dropna()
        return alpha
    ...
    def alpha_099(self):
        ...
        temp = ((self.close.rank(axis=1, pct=True)).rolling(window=5).cov(self.volume.rank(axis=1, pct=True)))
        alpha = (temp.rank(axis=1, pct=True)).iloc[-1, :]
        alpha = -alpha.dropna()
        return alpha
    ...
    def alpha_101(self):
        ...
        alpha = -(rank1 < rank4)
        alpha = alpha.iloc[-1, :].astype(floa).dropna()
        return alpha
    ...
    def alpha_103(self):
        ...
        ##
        alpha = (20 - self.low.iloc[-20:, :].apply(func_lowday)) / 20 * 100
        alpha = alpha.dropna()
        return alpha
    ...
    def alpha_109(self):
        ...
        sma1 = data.ewm(span=5, adjust=False).mean()
        sma2 = sma1.ewm(span=5, adjust=False).mean()
    ...
    def alpha_111(self):
        ...
        x = data1.ewm(span=5.5).mean()
        y = data1.ewm(span=2).mean()
        alpha = (x - y).iloc[-1, :]
        alpha = alpha.dropna()
        return alpha
    ...
    def alpha_113(self):
        ...
        rank1 = (data1.rolling(20).sum() / 20).rank(axis=1, pct=True)
        corr1 = self.close.iloc[-2:, :].corrwith(self.volume.iloc[-2:, :])
        data2 = self.close.rolling(5).sum()
        data3 = self.close.rolling(20).sum()
    ...
    def alpha_114(self):
        ...
        data2 = (((self.high - self.low) / (self.close.rolling(5).sum() / 5)) / (self.avg_price - self.close)).iloc[-1,
                :]
    ...
    def alpha_115(self):
        ...
        tsrank1 = ((self.high + self.low) / 2).iloc[-4:].rank()
        tsrank2 = self.volume.iloc[-10:].rank()
    ...
    #(RANK(DECAYLINEAR(CORR(VWAP,SUM(MEAN(VOLUME,5),26),5),7))-RANK(DECAYLINEAR(TSRANK(MIN(CORR(RANK(OPEN),RANK(MEAN(VOLUME,15)),21),9),7),8)))
        ...
        rank1 = (corr1.rolling(window=7).apply(func_decaylinear)).rank(axis=1, pct=True)
        rank2 = self.open_price.rank(axis=1, pct=True)
        rank3 = (self.volume.rolling(window=15).mean()).rank(axis=1, pct=True)
        rank4 = ((rank2.rolling(window=21).corr(rank3)).rolling(window=9).min()).rolling(window=7).apply(func_rank)
        rank5 = rank4.rolling(window=8).apply(func_decaylinear).rank(axis=1, pct=True)
    ...
    def alpha_122(self):
        ...
        temp = (log_close.ewm(span=12, adjust=False).mean()).ewm(span=12, adjust=False).mean()
        data = temp.ewm(span=12, adjust=False).mean()
...
#(RANK(DECAYLINEAR(CORR((VWAP), MEAN(VOLUME, 80), 17), 20)) / RANK(DECAYLINEAR(DELTA((CLOSE * 0.5 + VWAP * 0.5), 3), 16)))
        ##
        data1 = self.avg_price.rolling(17).corr(self.volume.rolling(80).mean())
...
#(RANK(DELCAYLINEAR(CORR(((HIGH + LOW) / 2), MEAN(VOLUME, 40), 9), 10)) / RANK(DELCAYLINEAR(CORR(RANK(VWAP), RANK(VOLUME), 7), 3)))
        ...
        data2 = self.volume.rolling(40).mean()
        data3 = data1.rolling(9).corr(data2)
        ...
        data1 = self.avg_price.rank(axis=1, pct=True)
        data2 = self.volume.rank(axis=1, pct=True)
        data3 = data1.rolling(7).corr(data2)
    ...
    def alpha_133(self):
        ...
        alpha = (20 - self.high.iloc[-20:, :].apply(func_highday)) / 20 * 100 \
                - (20 - self.low.iloc[-20:, :].apply(func_lowday)) / 20 * 100
        alpha = alpha.dropna()
        return alpha
    ...
    def alpha_135(self):
        ...
        data1 = (self.close/self.close.shift(20)).shift(1)
        alpha = (data1.ewm(alpha=1/20).mean()).iloc[-1, :]
        alpha = alpha.dropna()
        return alpha
    ...
    def alpha_136(self):
        ...
        ##
        data1 = -(self.close / self.close.shift() - 1).diff(3).rank(axis=1, pct=True)
    ...
    def alpha_138(self):
        ...
        data1 = self.low.iloc[-8:].rank()
        data2 = self.volume.rolling(60).mean().iloc[-17:].rank()
        data3 = data1.iloc[-5:].corr(data2.iloc[-5:])
        rank2 = data3.rolling(16).apply(func_decaylinear).iloc[-7:, :].rank(axis=0, pct=True).iloc[-1, :]
...
#         alpha = result.iloc[-1, :]
#         return alpha.dropna()
        data=self.close-self.close.shift(20)
        result = data.ewm(alpha=1/20).mean()
        alpha = result.iloc[-1, :]
        return alpha.dropna() 
    ...
    def alpha_152(self):
        ...
        #
        temp = ((self.close / self.close.shift(9)).shift()).ewm(span=17, adjust=False).mean()
        data1 = (temp.shift()).rolling(window=12).mean()
        data2 = (temp.shift()).rolling(window=26).mean()
    ...
    def alpha_153(self):
        ...
        alpha = ((self.close.rolling(3).mean() + self.close.rolling(6).mean() + self.close.rolling(
            12).mean() + self.close.rolling(24).mean()) / 4).iloc[-1, :]
        alpha = alpha.dropna()
        return alpha
    ...
    def alpha_155(self):
        ...
        sma1 = self.volume.ewm(alpha=2/13).mean()
        sma2 = self.volume.ewm(alpha=2/27).mean()
        data=sma1-sma2
        sma = sma1 -sma2-data.ewm(alpha=2/10).mean()
        alpha = sma.iloc[-1, :]
        alpha = alpha.dropna()
        return alpha
...
# 2) / ((OPEN * 0.15) + (LOW * 0.85))) * -1), 3))) * -1)
        ...
        rank1 = ((self.avg_price - self.avg_price.shift(5)).rolling(window=3).apply(func_decaylinear)).rank(axis=1,
                                                                                                            pct=True)
        temp = -((self.open_price * 0.15 + self.low * 0.85) - (self.open_price * 0.15 + self.low * 0.85).shift(2)) / (
                    self.open_price * 0.15 + self.low * 0.85)
        rank2 = (temp.rolling(window=3).apply(func_decaylinear)).rank(axis=1, pct=True)
    ...
    def alpha_157(self):
        ...
        cond_min = rank2 > 5
        rank2[cond_min] = 5
        tsrank1 = ((-((self.close / self.prev_close) - 1)).shift(6)).rolling(window=5).apply(func_rank)
    ...
    def alpha_158(self):
        ...
        temp1 = self.high - self.close.ewm(span=14, adjust=False).mean()
        temp2 = self.low - self.close.ewm(span=14, adjust=False).mean()
        alpha = ((temp1 - temp2) / self.close).iloc[-1, :]
        alpha = alpha.dropna()
        return alpha
    ...
    def alpha_160(self):
        ...
        cond = self.close <= self.close.shift(1)
        data1[~cond] = 0
        data2 = data1.ewm(alpha=1/20).mean()
        alpha = data2.iloc[-1, :]
        alpha = alpha.dropna()
        return alpha
    ...
    def alpha_163(self):
        ...
        data1 = (-1) * (self.close / self.close.shift() - 1) * self.volume.rolling(20).mean() * self.avg_price * (
    ...
    def alpha_164(self):
        ...
        alpha = (data3.ewm(alpha=2/13).mean()).iloc[-1, :]
        alpha = alpha.dropna()
        return alpha
    ...
    def alpha_170(self):
        ...
        data4 = self.high.rolling(5).sum()/5
        y = (data3 * self.high) / data4
        z = (self.avg_price - self.avg_price.shift(5)).rank(axis=0, pct=True)
        alpha = (x * y - z).iloc[-1, :]
        alpha = alpha.dropna()
        return alpha
    ...
    def alpha_175(self):
        ...
        data4 = (data3.rolling(6).mean()).iloc[-1, :]
        alpha = data4
        alpha = alpha.dropna()
        return alpha
    ...
    def alpha_176(self):
        ...
        data1 = (self.close - self.low.rolling(12).min()) / (self.high.rolling(12).max() - self.low.rolling(12).min())
    ...
    def alpha_180(self):
        ...
        ma = self.volume.rolling(20).mean()
    ...
    #COUNT((CLOSE>OPEN & BANCHMARKINDEXCLOSE>BANCHMARKINDEXOPEN)OR(CLOSE<OPEN & BANCHMARKINDEXCLOSE<BANCHMARKINDEXOPEN),20)/20 #####
        ...
        count = cond.rolling(window=20).apply(self.count_cond_182)
    ...
    def alpha_184(self):
        ...
        ##
        data1 = (self.open_price - self.close).shift()
        #data2 = self.open_price.iloc[-1, :] - self.close.iloc[-1, :]
        corr = data1.iloc[-200:].corrwith(self.close.iloc[-200:])
        alpha =(self.open_price - self.close).rank(axis=0, pct=True) + corr.rank(axis=0, pct=True)
        alpha = alpha.dropna()
        return alpha
    ...
    def alpha_187(self):
        ...
        cond = (self.open_price <= self.open_price.shift())
        data1 = self.high - self.open_price  # HIGH-LOW
    ...
    def alpha_189(self):
        ...
        ma6 = self.close.rolling(6).mean()
        alpha = (((self.close - ma6).abs()).rolling(6).mean()).iloc[-1, :]
        alpha = alpha.dropna()
        return alpha
    ...
    def alpha_191(self):
        ##### (CORR(MEAN(VOLUME,20), LOW, 5) + ((HIGH + LOW) / 2)) - CLOSE ####/chencheng
        volume_avg = self.volume.rolling(20).mean()
